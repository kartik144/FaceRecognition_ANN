...
{
	int nh=6,no=4,ni=960; // variables to store number of nodes in hidden layer,output layer and input layer respectively
	...
	public static void main(String [] args)throws IOException//main function
	{
		...
		while((s=br.readLine())!=null)//reads each input image of testing set
		{
			...
			for(int i=0;i<obj.no;i++)
				if(obj.t[i]==1)
				indexOfTarget=i;//stores the adress at which the user id of the person in image is stored in poses[] array
			
			System.out.println(obj.poses[indexOfOutput]+"\t\t"+file_name);// outputs the file name which has been classified along with the pose
